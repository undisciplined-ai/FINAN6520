...
3. **Document-wide terminology sweep**: Replace all references to "4×8 grid," "3×8 grid," or "matrix grid" language. The structure is no longer a fixed-dimension grid — it is an appendable list of parent categories. The initial 24 categories correspond to the 3 planes × 8 movement types, but additional parent categories can be appended without structural change. Update comments and docstrings accordingly (e.g., "building the 4x8 grid" → "building the category list"). Note: "Section 4.8" references (DDM subsection number) are coincidental — leave those untouched.
...
5. **REPLACE** `DEFAULT_MATRIX_GRID` (Section 1.4) with:
   ...
   - `PRESET_MATRIX_CONFIGS` dict with a BLANK preset defining all 24 parent categories (Sagittal×8, Frontal×8, Transverse×8). Each category has `'categories'` list with name, `weight: 0`, measurement_unit, exercise_examples. All weights default to 0 — the user populates weights when ready. Stub entries (name + comment "TBD") for GPP, STRENGTH, HYPERTROPHY, POWERLIFTING, FUNCTIONAL.
...
6. Update Section 1.5: Remove 'Neutral' from `MOVEMENT_PLANES` set and `MOVEMENT_PLANES_ORDERED` list. Remove `PRIORITY_OPTIONS` (see step 4). Update iteration comment to reflect appendable category list (see step 3).
...
20. **6.3 Program Balance**: Update.
    - Replace grid-based comments with category-list terminology (see step 3).
...
27. Replace `IMPLEMENTATION_ROADMAP` string content with updated version:
    - **Stage 1** (Constants): Updated test — verify 24 categories in BLANK preset (all weights = 0), MEASUREMENT_UNITS present, 'Neutral' absent from MOVEMENT_PLANES, PRIORITY_TARGETS and PRIORITY_OPTIONS absent.
...
- Default preset is BLANK (all 24 categories with weight = 0). GPP/STRENGTH/HYPERTROPHY/POWERLIFTING/FUNCTIONAL preset stubs included but not fully defined (marked TBD).
...
- `'repeat_flag'` and its backing `repeat_exercise` calculation removed from Program Balance (was the V1.0.1 field name at line 1105).
